    ...
    <footer>
        <div class="content">
            ...
            <button type="button" class="toggleBtn" on:click={() => (sideBarShow = !sideBarShow)}>List</button>
            <SideBar bind:show={sideBarShow} />
            <button type="button" class="toggleBtn" id="nextBtn" on:click={() => navigateItem('prev')} disabled={prevDisabled}>Previous</button>
            <span><p><b>{$pageNumber} of {$apiData.length}</b></p></span>
            <button type="button" class="toggleBtn" id="prevBtn" on:click={() => navigateItem('next')} disabled={nextDisabled}>Next</button>
            <button type="submit" class="toggleBtn" on:click={openModal} >End Test</button>
